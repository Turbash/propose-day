...
export function ImageGallery() {
  ...
  return (
    <div className="flex items-center justify-center bg-gradient-to-br from-pink-100 via-rose-100 to-red-100 min-h-screen font-sans px-4">
      <div className="relative h-[95vmin] w-[95vmin] sm:h-[85vmin] sm:w-[85vmin] max-h-[600px] max-w-[600px] overflow-hidden rounded-[20px] shadow-[0_2.8px_2.2px_rgba(0,0,0,0.02),0_6.7px_5.3px_rgba(0,0,0,0.028),0_12.5px_10px_rgba(0,0,0,0.035),0_22.3px_17.9px_rgba(0,0,0,0.042),0_41.8px_33.4px_rgba(0,0,0,0.05),0_100px_80px_rgba(0,0,0,0.07)]">
        {gsapReady &&
          images.map((image, i) => (
            <div
              key={image.url}
      ...
      <button
        className="absolute right-2 sm:right-[calc(50%-300px-50px)] top-1/2 z-[101] flex h-12 w-12 sm:h-16 sm:w-16 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-2 border-white/20 bg-white/95 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-white hover:border-white/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] active:scale-95 focus-visible:ring-4 focus-visible:ring-white/50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
        onClick={next}
        disabled={disabled}
